...
import { readBrowserOrders } from "@/lib/merchant/browser-orders";
import type { Order, Product, PurchaseAuthorization } from "@/types";
...
export function OrdersDashboard({
  ...
  initialAuthorizations: PurchaseAuthorization[];
  initialOrders: Order[];
}) {
  const [browserOrders, setBrowserOrders] = useState<Order[]>([]);

  useEffect(() => {
    function refreshBrowserOrders() {
      setBrowserOrders(readBrowserOrders());
    }

    refreshBrowserOrders();
    window.addEventListener("storage", refreshBrowserOrders);
    window.addEventListener("smartmerce:orders-updated", refreshBrowserOrders);
    return () => {
      window.removeEventListener("storage", refreshBrowserOrders);
      window.removeEventListener("smartmerce:orders-updated", refreshBrowserOrders);
    };
  }, []);

  const orders = useMemo(() => mergeOrders(initialOrders, browserOrders), [browserOrders, initialOrders]);
  const liveProducts = initialProducts
    .filter((product) => product.source === "internet")
    .sort((a, b) => a.name.localeCompare(b.name));
  const pendingPayments = initialAuthorizations.map((authorization) => ({
    authorization,
    product: initialProducts.find((product) => product.id === authorization.productId),
  }));
      ...
      <section className="mb-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_380px] lg:items-stretch">
        <div className="glass rounded-lg p-5 md:p-7">
          ...
          <h1 className="mt-5 max-w-4xl text-4xl font-semibold leading-tight text-ink sm:text-5xl">
            Track what the AI found, what you approved, and what was paid.
          </h1>
          <p className="mt-5 max-w-3xl text-base leading-7 text-muted">
            SmartMerce keeps a browser copy of completed purchases, so your order stays visible even when the demo server restarts.
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="glass overflow-hidden rounded-lg">
          ...
          <div className="divide-y divide-line">
            ...
            <TimelineBlock
              ...
              description="You approved the product and amount, but wallet payment is not complete yet."
            ...
            >
              {pendingPayments.slice(0, 6).map(({ authorization, product }) => (
                <Row
                  ...
                  meta={`${authorization.exactAmount.toFixed(2)} XSGD · ${authorization.id} · expires ${formatTime(authorization.expiresAt)}`}
                  badge="Waiting"
                  tone="amber"
                />
              ))}
        ...
        <aside className="space-y-6">
          <section className="glass rounded-lg p-5">
            ...
            <div className="mt-5 space-y-3 text-sm">
              {[
                ...
                ["Waiting", "The item is approved, but wallet payment is not complete."],
          ...
          <section className="glass rounded-lg p-5">
            ...
            <div className="mt-5 space-y-3 text-sm">
              {[
                `${initialProducts.filter((product) => product.inStock).length} items currently available`,
                  ...
                  <tr key={order.id}>
                    ...
                    <td className="px-5 py-4">
                      {order.transactionHash ? (
                        <a className="font-mono text-xs text-violet underline" href={explorerTransactionUrl(order.transactionHash)} target="_blank" rel="noreferrer">
                    ...
                    <td className="px-5 py-4 text-muted">{formatDateTime(order.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function mergeOrders(serverOrders: Order[], browserOrders: Order[]) {
  const merged = [...browserOrders, ...serverOrders];
