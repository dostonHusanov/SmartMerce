...
export default function MerchantPage() {
  ...
  const sortedProducts = [...products].sort((a, b) => {
    if (a.source === "internet" && b.source !== "internet") return -1;
    if (a.source !== "internet" && b.source === "internet") return 1;
    return a.name.localeCompare(b.name);
  });
  const liveProducts = sortedProducts.filter((product) => product.source === "internet");
    ...
    <main className="mx-auto max-w-7xl px-6 py-10">
      <section className="mb-8 grid gap-6 lg:grid-cols-[1fr_380px] lg:items-end">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-muted">
            The store view shows AI-picked products, purchases waiting for wallet payment, and orders that have verified payment proof.
          </p>
        </div>
      ...
      <div className="mb-6 grid gap-4 md:grid-cols-4">
        {([
          ...
          ["Waiting for payment", pendingPayments.length, Clock3],
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
        <section className="glass overflow-hidden rounded-lg">
          <div className="border-b border-line p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h2 className="text-sm font-semibold uppercase tracking-[0.18em] text-ink">Products SmartMerce Can Sell</h2>
              {liveProducts.length ? (
                <span className="rounded-full border border-accent/30 bg-accent/10 px-3 py-1 text-xs font-bold text-accent">
                  {liveProducts.length} live web
                </span>
              ) : null}
            </div>
          </div>
                  ...
                  <tr key={product.id} className="text-ink">
                    <td className="px-5 py-4">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted">
                        ...
                        {product.source === "internet" ? (
                          <span className="rounded-full border border-accent/30 bg-accent/10 px-2 py-0.5 text-[10px] font-bold uppercase text-accent">
                            AI picked
                          </span>
                        ) : null}
                      </div>
                    ...
                    <td className="px-5 py-4">
                      <span className={`rounded-full px-3 py-1 text-xs font-medium ${product.inStock ? "bg-accent/10 text-accent" : "bg-red-300/10 text-red-300"}`}>
                        {product.inStock ? (product.source === "internet" ? "Ready for checkout" : "Active") : "Out of stock"}
                      </span>
                    </td>
                  </tr>
                ))}
        ...
        <aside className="space-y-6">
          <section className="glass rounded-lg p-5">
            <h2 className="text-sm font-semibold uppercase tracking-[0.18em] text-ink">Approved, Waiting For Payment</h2>
            <div className="mt-5 space-y-3">
              {pendingPayments.length ? (
                pendingPayments.slice(0, 4).map(({ authorization, product }) => (
                  <div key={authorization.id} className="rounded-md border border-line bg-white/[0.03] p-4 text-sm">
                    ...
                    <div className="mt-1 text-xs text-muted">Shopper approved. Wallet payment still required.</div>
                    <div className="mt-2 font-mono text-[10px] text-muted">{authorization.id}</div>
                ...
                <div className="rounded-md border border-line bg-white/[0.03] p-4 text-sm text-muted">
                  When a shopper approves a product, it appears here until they finish the wallet payment.
                </div>
              )}
            </div>
          </section>
          <section className="glass rounded-lg p-5">
            <h2 className="text-sm font-semibold uppercase tracking-[0.18em] text-ink">Live Products Found By AI</h2>
            <div className="mt-5 space-y-3">
              {liveProducts.length ? (
                liveProducts.slice(0, 4).map((product) => (
                  <div key={product.id} className="rounded-md border border-line bg-white/[0.03] p-4 text-sm">
                    <div className="font-semibold text-ink">{product.name}</div>
                    <div className="mt-2 text-accent">{product.priceXsgd.toFixed(2)} XSGD</div>
                    <div className="mt-1 text-xs text-muted">{product.merchant}</div>
                  </div>
                ))
              ) : (
                <div className="rounded-md border border-line bg-white/[0.03] p-4 text-sm text-muted">
                  Run a shopper search first. Live products discovered by the AI will appear here.
                </div>
              )}
            </div>
          </section>
